...
- Throughout the repository, the name `Go2Testing` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of Go2Testing (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/Go2Testing
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/Go2Testing/Go2Testing/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/Go2Testing"
    ]
}
```
